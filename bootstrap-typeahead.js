...
//        	subitemClass: 'subitem', //class that will be added to subitems
...
function ($) {
    ...
    var isFn = function isFunctionC(object) {
    ...
    var Typeahead = function (element, options) {
        ...
        this.subitemClass = this.options.subitemClass || 'subitem';
    ...
    Typeahead.prototype = {
            ...
            items = $.grep(items, function (item) {
                if(that.options.searchIn){
					if($.isArray(that.options.searchIn)){
                     ...
                     if($.isFunction(that.options.display)){
                        return that.matcher(that.options.display(item), item);
                    }else{
        ...
        render: function (items) {
            ...
            items = $(items).map(function (i, item) {
                ...
                var ancor = i.find('a');
                        if(that.options.display){
                            if(isFn(that.options.display)){
                                ancor.html(that.highlighter(that.options.display(item)));
                            }else{
...
//                if(subitemIcon.length > 0){
                    if(item.hasOwnProperty('SubItem')){
                        if(item.SubItem > 0){
                            if (item.hasOwnProperty('RegionId')) {
                                $(i[0]).addClass('subitem city-item');
                            }else {
                                $(i[0]).addClass('subitem airport-item');
                    ...
                    if(item.hasOwnProperty('SubItem')){
                        if(item.SubItem<1){
                            if (item.hasOwnProperty('RegionId') && item.RegionId>0 && that.$element.hasClass('no-region')) {
                                return;
                            }
                            else if (item.hasOwnProperty('RegionId') && item.RegionId>0) {
                                $(i[0]).addClass('region-item');
                            }else {
                                $(i[0]).addClass('city-airport-item');
                            }
                        }else {
